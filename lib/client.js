			...
			const arm = () => {
				...
				const remount = new MutationObserver(() => {
					...
					if (!sc.isConnected) {
						...
						// and any expanded-state ref must not outlive the subtree)
				...
				armed = { sc, mo, remount, resize, onScroll, retry: null };
				schedule();
				return true;
			};
			// scroller not mounted yet (SPA lazy render): watch for it instead of
			// giving up after one 1s retry
			if (!arm()) {
				const retry = new MutationObserver(() => {
					if (document.querySelector("[data-conversation-scroll]") !== null) {
						retry.disconnect();
						arm();
					}
				});
