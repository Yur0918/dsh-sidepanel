...
//
// Coexistence with the native right rail (DSH 0.1.5+ inject package
// @deepseek-ai/dsh-client-ui-sidebar-right): a data-attribute probe detects
// the rail in the DOM and, under the default "auto" policy, an absent→present
// transition of the rail collapses this panel once (the user can re-expand);
// the "ignore" policy and every probe miss on older hosts keep the legacy
// behavior unchanged.
//
...
window.__ModuleLoader__.load({
	...
	factory: function (require) {
		...
		// ── native right-rail probe (coexistence with DSH 0.1.5+) ────────────
		// Candidates are data-* attributes OWNED by the host package
		// @deepseek-ai/dsh-client-ui-sidebar-right, verified verbatim in both
		// shipped copies:
		//   0.1.5-rc.2 (~/.dsh/profiles/node_modules/@deepseek-ai/dsh-client-ui-sidebar-right/lib/client.js):
		//     panel L856/L857, expand L214, toggle L837, mode L822
		//   0.1.6-alpha.1 (.cluster unpack dsh016-ex/deepseek-ai-dsh-client-ui-sidebar-right-0.1.6-alpha.1/package/lib/client.js):
		//     panel L871/L872, expand L229, toggle L852, mode L837
		// The rail panel element stays mounted while collapsed ("translated off
		// the frame's right edge", SidebarRight doc comment), so the panel
		// attribute is the primary presence signal; the rest are fallbacks in
		// case a future build renames one. Version-number checks are forbidden
		// by design — presence in the DOM IS the capability test.
		const NATIVE_RAIL_PROBE_SELECTORS = [
		...
		function sanitizeState(raw) {
			...
			// native right-rail policy: only a NON-default value is kept, so states
			// from before this field (and "auto" itself) serialize without the key
			// and old hosts / untouched users keep byte-identical persisted state
			if (src.nativeRail === "ignore") state.nativeRail = "ignore";
			// per-session model overrides ride along as chatModel.<sessionId> keys
			for (const key of Object.keys(src)) {
				if (key.startsWith("chatModel.") && typeof src[key] === "string" && src[key].trim() !== "") {
					state[key] = src[key];
				}
			}
			return state;
		...
		// native-rail DOM probe: one comma-joined querySelector over the grounded
		// candidate list; missing document (SSR/tests) and old hosts yield false
		function probeNativeRailHit(doc) {
			if (doc === null || doc === undefined || typeof doc.querySelector !== "function") return false;
			return doc.querySelector(NATIVE_RAIL_PROBE_SELECTORS.join(",")) !== null;
		}

		// pure decision: probe hit + policy + panel state → action. Missing policy
		// (old persisted state without the field) behaves exactly as "auto".
		function decideNativeRailAction(hit, policy, open) {
			if (hit !== true) return "none";
			if (policy === "ignore") return "none";
			return open === true ? "collapse" : "none";
		}

		// edge-triggered observation step: the FIRST probe of a mount episode only
		// sets the baseline (a persisted open or an explicit user expand must never
		// be undone at mount), unchanged readings are no-ops, and only an
		// absent→present transition can yield. present→absent never re-expands —
		// the panel comes back on the next user action, not on rail flapping.
		function railObservationStep(prev, hit, policy, open) {
			if (prev === null || prev === hit) return { next: hit, action: "none" };
			return { next: hit, action: decideNativeRailAction(hit, policy, open) };
		}
		...
		function sweepStaleComp(item, keep) {
			...
			for (const el of parent.querySelectorAll("[data-dsp-rest-comp]")) {
				...
				if (prev === "empty" || prev === null) el.style.removeProperty("margin-top");
		...
		function applyRestComp(item, px) {
			...
			// marker presence means "the original margin is saved inside it";
			// there is no second state — re-applying must not overwrite the
			// saved original with an intermediate value
			if (nx.getAttribute("data-dsp-rest-comp") === null) {
		...
		function clearRestComp(item) {
			const nx = item.nextElementSibling;
			if (nx !== null && nx.getAttribute("data-dsp-rest-comp") !== null) {
		...
		exports._pure = {
			...
			sweepStaleComp, applyRestComp, clearRestComp,
			NATIVE_RAIL_PROBE_SELECTORS, probeNativeRailHit, decideNativeRailAction, railObservationStep,
...
`;
		...
		function useFrameTrack(open, widthRef, setMode, onRailObservation) {
			const panelRef = React.useRef(null);
			const applyRef = React.useRef(null);
			React.useEffect(() => {
				const panel = panelRef.current;
				if (panel === null) return;
				// the panel portals to a body-level host, so the frame cannot be
				// reached via closest(); [data-shell-overlay] is React-owned, stable
				// and unique in the document — its parent is the grid frame
				...
				const apply = () => {
					...
					setMode(mode);
					// native right-rail coexistence: this panel's own frame observers
					// (style mutations + resizes) are also the probe's pulse — the rail
					// track lives in the same React-owned grid template, so every rail
					// mount/unmount/expand/collapse walks through here. No new timer,
					// no new observer.
					if (typeof onRailObservation === "function") onRailObservation();
				};
		...
		// Only ONE user row may be sticky at a time — CSS sticky on every row
			...
			function mainChatApplySticky() {
				...
				for (let i = 0; i < stickyCache.items.length; i++) {
					...
					if (!item.classList.contains("dsp-main-collapsed") && !naturalH.has(item)) naturalH.set(item, item.getBoundingClientRect().height);
		...
		const IconRefresh = svg([
			React.createElement("path", { key: "p1", d: "M13.5 8a5.5 5.5 0 1 1-1.6-3.9", stroke: "currentColor", strokeWidth: 1.4, strokeLinecap: "round" }),
			React.createElement("path", { key: "p2", d: "M13.7 1.8v2.7h-2.7", stroke: "currentColor", strokeWidth: 1.4, strokeLinecap: "round", strokeLinejoin: "round" })
		]);
		const IconCopy = svg([
		...
		function ArtifactsView({ sessionId, includeScan, onCount, onToggleScan }) {
			...
			const reload = React.useCallback((silent) => {
				...
				apiGet("/sidepanel/artifacts?sessionId=" + encodeURIComponent(sessionId) + "&scan=1")
					.then((res) => {
						setError(""); // a later success clears an earlier failure banner
						setData(res);
					})
			...
			// badge mirrors what the 全部 view will actually list: with 「含扫描来源」
			// off the scan rows are filtered out of the view, so counting them in
			// the badge would promise files it does not show
			React.useEffect(() => {
				if (data === null) return;
				const c = data.counts || {};
				onCount(includeScan === true ? (c.total || 0) : (c.produced || 0));
			}, [data, includeScan]);
			React.useEffect(() => {
				if (sessionId === undefined || sessionId === "") return;
			...
			const openPreview = (item) => {
				...
				apiGet("/sidepanel/file?sessionId=" + encodeURIComponent(sessionId) + "&path=" + encodeURIComponent(item.path))
					.then((payload) => {
						// ignore a stale response for a preview the user already switched
						setPreview((current) => (current !== null && current.path === item.path ? { path: item.path, loading: false, payload } : current));
					})
					.catch((e) => {
						setPreview((current) => (current !== null && current.path === item.path ? { path: item.path, loading: false, payload: { ok: false, error: e instanceof Error ? e.message : String(e) } } : current));
					});
			};

			const copyPath = (item) => {
				const done = () => { setCopied(item.path); setTimeout(() => setCopied(""), 1400); };
				const failed = () => { setCopied(""); };
				if (Prim !== null && typeof Prim.writeClipboard === "function") {
					Prim.writeClipboard(item.path).then(done).catch(failed);
				} else if (navigator.clipboard !== undefined) {
					navigator.clipboard.writeText(item.path).then(done).catch(failed);
				}
			};
			...
			return React.createElement(React.Fragment, null,
				React.createElement("div", { className: "dsp-toolbar" },
					...
					React.createElement("button", {
						className: "dsp-iconbtn", type: "button", "aria-label": T.refresh, title: T.refresh,
		...
		function chatStoreOf(sessionId) {
			...
			if (store === undefined) {
				...
				if (chatStores.size >= 12) {
					// FIFO eviction must not drop a store with live subscribers
					// (its chat would silently reset mid-use)
					for (const key of chatStores.keys()) {
						if (chatStores.get(key).subs.size === 0) { chatStores.delete(key); break; }
					}
				}
			...
			React.useEffect(() => {
				...
				es.onmessage = (ev) => {
					...
					if (frame.type === "hello") {
						...
						// every (re)connect reconciles with the host transcript: frames
						// missed while the EventSource was down are backfilled here
						apiGet("/sidepanel/chat/state?sessionId=" + encodeURIComponent(sessionId))
							.then((res) => {
								setMeta((m) => Object.assign({}, m, { childId: res.childId ?? frame.childId, model: res.model, provider: res.provider }));
								const local = stateRef.current;
								const hostCount = (res.messages || []).length;
								const stale = local.phase === "streaming" && res.running !== true;
								if (hostCount > local.messages.length || (stale && hostCount >= local.messages.length)) {
									dispatch({ type: "restore", messages: res.messages, running: res.running, partial: res.partial });
								}
							})
							.catch(() => {});
				...
				React.createElement("div", { className: "dsp-composer" },
					...
					React.createElement("div", { className: "dsp-composer-side" },
						...
						modelsOpen ? React.createElement("div", { className: "dsp-model-pop", role: "listbox", "aria-label": T.model,
							onKeyDown: (e) => {
								// Esc closes the popover first, not the whole panel
								if (e.key === "Escape") {
									e.preventDefault();
									e.stopPropagation();
									setModelsOpen(false);
							...
							React.createElement("button", {
								...
								onClick: () => {
									// follow-main clears this session's override (falls back to
									// the global pick, then to the inherited chat model)
									panelStore.set({ ["chatModel." + (sessionId || "")]: null });
							...
							models.map((m) => React.createElement("button", {
								...
								onClick: () => {
									panelStore.set({ ["chatModel." + (sessionId || "")]: m.id });
					...
					React.createElement("textarea", {
						...
						placeholder: T.chatPlaceholder, "aria-label": T.chatPlaceholder,
						onChange: (e) => setDraft(e.target.value),
						onKeyDown: (e) => {
							// isComposing: Enter during zh/ja IME composition confirms the
							// candidate, it must not send
							if (e.key === "Enter" && !e.shiftKey && e.nativeEvent.isComposing !== true) {
								e.preventDefault();
								send(draft);
							}
						}
		...
		function SidePanelInner({ useSessions }) {
			...
			const widthRef = React.useRef(state.width);
			widthRef.current = state.width;
			// native right-rail coexistence probe: runs on the frame observers'
			// rhythm (see useFrameTrack); baseline is set by the first reading of
			// each mount episode, only an absent→present rail transition can yield
			// (collapse once, user can re-expand), and a disappearing rail never
			// re-expands the panel (no flapping)
			const railProbeRef = React.useRef(null);
			const onRailObservation = React.useCallback(() => {
				const hit = probeNativeRailHit(typeof document !== "undefined" ? document : null);
				// live open state (not a stale closure): a closed panel never issues
				// a redundant collapse write when the rail appears
				const step = railObservationStep(railProbeRef.current, hit, panelStore.get().nativeRail, panelStore.get().open === true);
				railProbeRef.current = step.next;
				if (step.action === "collapse") set({ open: false });
			}, []);
			const { panelRef, applyRef } = useFrameTrack(state.open, widthRef, setMode, onRailObservation);
			...
			const onResizeMove = (e) => {
				...
				// drag re-applies geometry directly through the ref — committing to
				// the store per pixel would re-render React and rewrite localStorage
				widthRef.current = clampPanelWidth(dragRef.current.startW + dx);
				if (applyRef.current !== null) applyRef.current();
			};
			const onResizeUp = (e) => {
				dragRef.current.dragging = false;
				set({ width: widthRef.current }); // single commit + persist on release
			...
			return React.createElement("aside", {
				...
				React.createElement("div", { className: "dsp-body" },
					// both views stay mounted: switching tabs must not tear down the
					// chat's SSE stream or the artifacts poll; hidden only toggles paint
					React.createElement("div", { hidden: state.tab !== "art", style: { flex: 1, minHeight: 0, display: state.tab === "art" ? "flex" : "none", flexDirection: "column" } },
						React.createElement(ArtifactsView, { sessionId, includeScan: state.includeScan, onCount: setArtCount, onToggleScan })),
					React.createElement("div", { hidden: state.tab !== "chat", style: { flex: 1, minHeight: 0, display: state.tab === "chat" ? "flex" : "none", flexDirection: "column" } },
