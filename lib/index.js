...
// single source of truth: the addon's own package.json (fallback keeps this
// module loadable even if the manifest is unreadable in a test sandbox)
...
// decode only the frames past fromBytes (append-only log growth); returns the
// new text and the consumed byte boundary. Logs append whole batches per
// frame, so every frame boundary is also a line boundary.
function decodeZstdLogFrom(buf, fromBytes) {
	const { frames } = parseZstdFrames(buf);
	const parts = [];
	let consumed = fromBytes;
	for (const [start, end] of frames) {
		if (end <= fromBytes) continue;
		if (start < fromBytes) break; // torn/partial coverage: caller refolds fully
		try {
			parts.push(zstdDecompressSync(buf.subarray(start, end)));
			consumed = end;
		} catch {
...
// feed one batch of decoded log text into a fold (the line loop of the old
// monolithic foldSessionLog); a partial trailing line is dropped and the
// caller is expected to re-feed it once the next batch completes it
function feedSessionText(fold, text, openCalls = new Map()) {
	let started = 0;
...
function finalizeSessionFold(fold) {
...
function foldSessionLog(text) {
	...
	feedSessionText(fold, text, openCalls);
	return finalizeSessionFold(fold);
}
...
// session-file lookup cache: findSessionFile walks every project dir of every
// route root, which the 5s artifacts poll would repeat per open panel. Short
// TTL; misses (not-yet-created sessions) are cached even shorter.
const fileCache = new Map(); // sessionId -> {till, found}
const FILE_TTL_MS = 15000;
const FILE_MISS_TTL_MS = 4000;

async function findSessionFile(sessionId) {
	if (typeof sessionId !== "string" || !/^[\w.-]{1,120}$/.test(sessionId)) return null;
	const cached = fileCache.get(sessionId);
	if (cached !== undefined && cached.till > Date.now()) return cached.found;
	const found = await findSessionFileUncached(sessionId);
	fileCache.set(sessionId, { till: Date.now() + (found === null ? FILE_MISS_TTL_MS : FILE_TTL_MS), found });
	if (fileCache.size > 200) fileCache.delete(fileCache.keys().next().value);
	return found;
}

async function findSessionFileUncached(sessionId) {
...
async function readSessionFold(sessionId) {
	...
	if (cached !== undefined && cached.mtimeMs === stat.mtimeMs && cached.size === stat.size) {
		return cached.mismatch === true ? Object.assign({}, cached.fold, { mismatch: true }) : cached.fold;
	}
	let fold;
	let openCalls;
	let decoded;
	const incremental = cached !== undefined
		...
		&& stat.size > cached.decoded
		&& stat.mtimeMs >= cached.mtimeMs
		&& file.endsWith(".zstd");
	if (incremental) {
		// append-only zstd growth: decode only the new frames and keep folding
		// into the cached object instead of re-decompressing the whole log
		fold = cached.fold;
		openCalls = cached.openCalls;
		decoded = cached.decoded;
	} else {
		fold = createSessionFold();
		openCalls = new Map();
		decoded = 0;
	}
	...
	if (file.endsWith(".zstd")) {
		const decodedNew = decodeZstdLogFrom(buf, decoded);
		text = decodedNew.text;
		decoded = decodedNew.consumed;
	} else {
		text = buf.toString("utf8");
		decoded = stat.size;
	}
	if (incremental && text === "") {
		// mtime moved but no new complete frame yet (torn tail): keep cache
		foldCache.set(file, Object.assign({}, cached, { mtimeMs: stat.mtimeMs, size: stat.size }));
		return fold;
	}
	feedSessionText(fold, text, openCalls);
	finalizeSessionFold(fold);
	const mismatch = fold.header !== null && fold.header.id !== "" && fold.header.id !== sessionId;
	if (mismatch) {
		// refuse to serve, but cache the verdict so each request does not
		// re-read + re-decode the whole log until the file changes again
		foldCache.set(file, { mtimeMs: stat.mtimeMs, size: stat.size, decoded, fold, openCalls, mismatch: true });
		return Object.assign({}, fold, { mismatch: true });
	}
	foldCache.set(file, { mtimeMs: stat.mtimeMs, size: stat.size, decoded, fold, openCalls });
...
// The artifacts panel polls every 5s; re-walking the workspace that often is
// waste. Cache the raw scan for a short TTL and re-apply the produced-file
// exclusion per call (tool items may change within the TTL window).
const SCAN_CACHE_TTL_MS = 30_000;
const scanCache = new Map();
async function cachedWorkspaceScan(cwd, sinceMs, excludeAbs) {
	const key = cwd + "|" + sinceMs;
	let entry = scanCache.get(key);
	if (entry === undefined || entry.till <= Date.now()) {
		const raw = await scanWorkspace(cwd, sinceMs, new Set());
		entry = { till: Date.now() + SCAN_CACHE_TTL_MS, items: raw };
		scanCache.set(key, entry);
		if (scanCache.size > 8) scanCache.delete(scanCache.keys().next().value);
	}
	return entry.items.filter((hit) => !excludeAbs.has(hit.abs));
}
...
async function buildArtifacts(sessionId, includeScan) {
	...
	const byAbs = new Map(); // rawPath variants (a.txt vs ./a.txt) collapse onto one abs
	for (const [rawPath, entry] of fold.produced) {
		...
		if (existing === undefined) {
			...
			// merge duplicate spellings of the same file into one artifact row
			for (const op of entry.ops) if (!existing.ops.includes(op)) existing.ops.push(op);
			for (const tool of entry.tools) existing.tools.add(tool);
			existing.count += entry.count;
			existing.firstTime = Math.min(existing.firstTime, entry.firstTime);
			existing.lastTime = Math.max(existing.lastTime, entry.lastTime);
			if (entry.ok) existing.ok = true;
		}
	}
	for (const [abs, entry] of byAbs) {
		producedAbs.add(abs);
	...
	let scanned = 0;
	if (includeScan && cwd !== "") {
		const scan = await cachedWorkspaceScan(cwd, createdAt, producedAbs);
		scanned = scan.length;
...
async function ensureParentAgent(sessionId) {
	...
	if (pending === undefined) {
		pending = (async () => {
			...
			if (fold.mismatch === true) throw new Error("session-id-mismatch");
		...
		// subscribe success AND failure: a bare .finally() derives a second
		// promise whose rejection is unhandled and can kill the process
...
async function chatSend(parentId, text, model) {
	...
	if (!childAccum.has(childId)) {
		if (childAccum.size > 200) childAccum.clear(); // rotated children accumulate across model switches
		childAccum.set(childId, { running: true, partial: "" });
	}
...
async function chatStop(parentId) {
	...
	for (const childId of ids) {
		...
		try {
			await subagents.interrupt(childId, { kind: "user", parentSessionId: parentId });
			stopped += 1;
		} catch (error) {
			return { ok: false, error: error instanceof Error ? error.message : String(error) };
		}
...
async function chatState(parentId) {
	...
	// findSideChild would re-query listChildren on a miss; we just did that —
	// fall back to the in-process cache instead of a second 15s-timeout IPC
	let childId = childIds.length > 0 ? childIds[childIds.length - 1] : (chatByParent.get(parentId) ?? null);
	if (childId === null) {
	...
	for (const id of known) {
		...
		if (fold === null || fold.mismatch === true) continue;
...
function onSessionEvent(session, event) {
	...
	if (event?.type === "assistant/chunk" && data0(event)?.chunk?.type === "text-delta" && acc !== undefined) {
		const t = event.data.chunk.text;
		if (typeof t === "string") acc.partial += t;
	}
	...
	try {
		routeDispose = pluginCtx.webServer.register({
			...
			try {
				...
				// a destroyed request (body over cap) has no writable response
				if (res.writableEnded || res.headersSent === true && status >= 500) return;
				try {
					sendJson(res, status, { ok: false, error: error instanceof Error ? error.message : String(error) });
				} catch {
					// socket gone: nothing to answer
				}
			}
		}
		});
